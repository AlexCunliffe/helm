...
The mirror reads every page before it changes stored rows. It supports at most 1000 returned events and 20 pages per sync, with a bounded event payload. A failed page, invalid snapshot, or exceeded limit leaves the old mirror intact. Narrow the calendar if a limit is reported. Timed events that overlap the lower boundary remain visible, including long ongoing meetings. Matching event IDs retain their row identity and prep state; missing and duplicate rows are removed. Google defines its lower time bound by event end and returns `nextPageToken` when more pages remain. See the [Google event-list reference](https://developers.google.com/workspace/calendar/api/v3/reference/events/list).
